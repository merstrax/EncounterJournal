...
function EncounterJournal_OnShow(self)
	...
	local instanceID = 0 --mapID and EJ_GetInstanceForMap(mapID) or 0;
	local _, instanceType, difficultyID = "", 0, 0; --GetInstanceInfo();
	...
	elseif ( EncounterJournal.overviewDefaultRole and not EncounterJournal.encounter.overviewFrame.linkSection ) then --self.encounter.overviewFrame:IsShown() and 
		local spec, role;

		spec = 0;--GetSpecialization();
		if (spec) then
			role = "DAMAGER"--GetSpecializationRole(spec);
		else
			role = "DAMAGER";
		end
...
function EncounterJournal_ListInstances()
	...
	local otherInstanceID = false; --EJ_GetInstanceByIndex(1, not showRaid);
...
function EJSuggestTab_GetPlayerTierIndex()
	return 0--GetEJTierDataTableID(GetExpansionForLevel(UnitLevel("player")));
end
